...
namespace Snappy.Common.Client.Tests.Models.InputModels
{
    ...
    {
        [TestCase("name", "label", true)]
        [TestCase("name", "label", false)]
        public void SelectInputModel(string name, string labelKey, bool isRequired)
        {
            var model = new SelectInputModel(name, labelKey, isRequired);
